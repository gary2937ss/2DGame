...
//			
//			tile[2] = new Tile();
//			tile[2].image = ImageIO.read(getClass().getResourceAsStream("/tiles/load.png"));
//			
//			
//			tile[3] = new Tile();
...
//		}
	...
	public void loadMap(String filePath) {
		...
		try {
			InputStream is = getClass().getResourceAsStream(filePath); //導入寫入的文件檔
			BufferedReader br =new BufferedReader(new InputStreamReader(is));   //再用Buffered來讀取他
			...
			while(col < gp.maxWorldCol && row < gp.maxWorldRow) {
				...
				String line = br.readLine(); //讀取一行文字，儲存在line中，並不斷循環。
	...
	public void draw(Graphics2D g2) {
		...
		while(worldCol < gp.maxWorldCol && worldRow < gp.maxWorldRow) {
			...
			int worldX = worldCol * gp.tileSize;  //worldX是地圖上的位置
